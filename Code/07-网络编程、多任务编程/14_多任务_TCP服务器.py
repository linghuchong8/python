...
def handle_client(client_socket: socket, client_addr: tuple):
    client_socket.send("欢迎光临【红浪漫】聊天室\n".encode("UTF-8"))
    # -------------------------------------------------------------------------->>
    # 循环接收消息，并回复
    while True:
        ...
        if client_data:
            ...
            print("收到来自【{}】的数据：{}".format(client_addr, msg))
            # 服务器回复客户端消息
            client_socket.send("收到\n".encode("utf-8"))
        else:
            # 收到空内容，说明客户端已断开链接
            print("客户端已断开：", client_addr)
...
if __name__ == '__main__':
    ...
    while True:
        ...
        thread = threading.Thread(target=handle_client, args=(tcp_client, tcp_client_addr))
        thread.daemon = True
        thread.start()
