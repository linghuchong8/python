...
if __name__ == '__main__':
    ...
    # 示例用法 (timeout: 1. 1s没收到数据就断开，0s立即断开，None 无限等待)
    sp = SerialDevice("COM24", baud_rate=115200, timeout=None)  # 替换为您的串口名称、波特率和超时时间
    sp.open()
    sp.write(b"Hello, Serial!\n")  # 发送数据
    # print(sp.read(20))  # 读取10个字节的数据
    print(sp.readline())  # 读取一行数据
    sp.close()
